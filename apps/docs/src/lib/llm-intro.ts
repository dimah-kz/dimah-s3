...
Security (app-owned; the library does not do this): never expose S3 credentials; the client sends a route name and the server owns keys — scope them with \`object\` / \`guard\`; trust \`onConfirmed\` (HeadObject, including multipart complete) for size and type, not the presign body.
`;
}

...
Security (app-owned; the library does not do this): never expose S3 credentials; the client sends a route name and the server owns keys under the route \`keyPrefix\`; prefer one feature per named route (combine upload/download/delete only when those callers share the key namespace); scope per-user folders with \`object\` / \`guard\` and ownership with \`db()\` or a download/delete \`guard\`; trust \`onConfirmed\` (HeadObject, including multipart complete) for size and type, not the presign body. \`fileTypes\` is the S3 Content-Type header and filename, not a byte sniff. Without \`db()\`, download can presign unconfirmed keys under \`keyPrefix\`.
`;
}

...
- `createS3Client` — browser/isomorphic client implementing `S3Api` (+ client plugins). Exposes `$ERROR_CODES`, `$fetch`, `$Infer`. `baseURL` wins over `basePath`. Route `expiresIn` defaults to **600** seconds (`S3_DEFAULT_EXPIRES_IN`) on the server.

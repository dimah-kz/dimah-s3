...
- `createS3Client` — browser/isomorphic client implementing `S3Api` (+ client plugins). Exposes `$ERROR_CODES`, `$fetch`, `$Infer`. `baseURL` wins over `basePath`. Client `expiresIn` defaults to **600** seconds (`S3_DEFAULT_EXPIRES_IN`).
- `DimahS3Error` — extends better-call `APIError` with the same `(status, body)` constructor; JSON `{ message, code?, params? }`. Use `DimahS3Error.from(status, S3_ERROR_CODES.FORBIDDEN)` or `isDimahS3Error` / `isAPIError`. Catalog: [Errors](/docs/server/errors).
...
The browser client (`createS3Client`) uses nested calls (`api.download(key)`,
`api.multipart.init`). The server `s3.api` is the better-call map
(`s3.api.download({ query })`, plus nested aliases `s3.api.multipart.init`).

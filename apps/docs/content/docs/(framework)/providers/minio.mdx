---
title: MinIO
description: Required path-style endpoint and CORS for MinIO.
---
...
Set the shared `S3_*` env vars in [Quickstart](/docs/quickstart), then apply the extras below.

## Path style (required)

Add **`forcePathStyle: true`** to the Quickstart `s3Sdk`. Without it, typical MinIO installs fail because object URLs must look like `http://host:9000/bucket/key`.
...
The default is `*` — fine for local development. Set an explicit origin when the app and MinIO run on different hosts, or when you lock CORS down. An `https://…` app talking to `http://localhost:9000` is still cross-origin, so the browser enforces CORS either way.

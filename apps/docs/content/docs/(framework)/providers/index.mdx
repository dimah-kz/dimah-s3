...
Presigned POST (R2 is the usual case). Leave `resolveObjectAcl` off unless
the provider actually supports object ACL — it adds a GetObjectAcl round-trip
on confirm.

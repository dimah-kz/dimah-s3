...
Works with any S3-compatible provider (AWS S3, R2, MinIO, …). Provider-specific env names: [Server setup](/docs/server/setup). Quirks (R2 needs `upload.method: "PUT"`, no ACL, …): [Providers](/docs/providers).
...
Then mount your layout inside the provider:
...
Mount it once near your app root so hooks and UI components can access the same client.

### UI setup [step]

Install the UI package (or a registry item), set theme variables, and mount the toaster:

...
Interactive runs prompt for a framework (`nextjs`, `vite`, or `hono`). Default is Next.js; pass `--template vite` or `--template hono` to skip the prompt. Then `cd my-app`, fill `S3_*` in `.env`, and start the app. The Next.js scaffold puts app code under `src/` by default (`--no-src` to flatten). Browser uploads need bucket CORS — see [Providers](/docs/providers).

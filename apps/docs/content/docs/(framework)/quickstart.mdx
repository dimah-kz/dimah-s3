...
Then `cd my-app`, fill `S3_*` in `.env`, and start the app. The scaffold puts app code under `src/` by default (`--no-src` to flatten). Browser uploads need bucket CORS — see [Providers](/docs/providers).

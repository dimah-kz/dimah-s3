...
Set bucket CORS for your origin — [Amazon S3](/docs/providers/aws-s3),
[Cloudflare R2](/docs/providers/cloudflare-r2), or [MinIO](/docs/providers/minio).

<Callout title="Cloudflare R2">
  Set `upload.method` to `"PUT"`. See [Cloudflare
...
export const { GET, POST, PUT, PATCH, DELETE } = toNextJsHandler(s3); // [!code highlight]
```
...
const app = express();
app.all("/api/s3/*", toExpressHandler(s3)); // [!code highlight]
...
import { Toaster } from "@dimah-s3/ui"; // [!code ++]
import { S3Provider } from "@/lib/s3-client"; // [!code ++]
...
```tsx title="app/layout.tsx"
import { Toaster } from "@/components/ui/toast"; // [!code ++]
import { S3Provider } from "@/lib/s3-client"; // [!code ++]
...
Selecting a file presigns a secure URL and uploads directly to your bucket.
  ...
  routes: {
    avatar: route({
      guard: async ({ request }) => {
        ...
        if (!session) throw errors.unauthorized(); // [!code ++]
      }, // [!code ++]
      upload: {
        fileTypes: ["image/*"],
...
const upload = useUpload({
  route: "avatar",
  onSuccess: (results) => {
    setValue("avatarKey", results[0]?.key); // [!code highlight]
  },
});
```
...
export function AvatarPreview({ avatarKey }: { avatarKey: string }) {
  const { url } = useObjectUrl({
    route: "avatar",
    objectKey: avatarKey,
    disposition: "inline", // [!code highlight]
  });

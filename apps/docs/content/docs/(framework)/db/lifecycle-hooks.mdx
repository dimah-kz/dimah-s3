...
Route membership is still the server `keyPrefix` check (`INVALID_KEY`). These
plugin guards do not compare the stored `route` column to the request route.
They look up the row by `(bucket, key)` and compare **scope**.
...
- **Upload** — Upserts a `pending` row on presign / multipart init.
- **Upload confirmation** — Verifies `pending` state (or refreshes an
  already-active overwrite), records verified `contentLength` from
  `HeadObject`, and returns the `active` row.

...
const { purged } = await purgeStalePendingObjects({
  client: dimahS3Db,
  olderThanMs: 24 * 60 * 60 * 1000, // 24 hours // [!code highlight]
  s3: awsS3,
});
...
Run it as a server-side scheduled job. A daily Vercel Cron, GitHub Actions
workflow, or Node cron worker is a common starting point; choose an interval
that matches how long your application should retain pending uploads.

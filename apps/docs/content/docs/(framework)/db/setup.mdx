...
description: Install @dimah-s3/db, add its schema, and register the db() plugin.
---
...
export const s3 = dimahS3({
  ...
  plugins: [
    db({
      // [!code ++]
      client: dimahS3Db, // [!code ++]
      resolveScope: async (request) => {
        // [!code ++]
        const session = await getSession(request); // [!code ++]
        return session ? `user:${session.userId}` : null; // [!code ++]
      }, // [!code ++]
    }), // [!code ++]
  ],
  routes: {
...
```ts
s3.db.objects.listByScope({ scope: "user:123" }); // [!code highlight]
```
...
<Callout type="warn" title="Features stay opt-in">
  ...
  ([combining operations](/docs/server/routes#combining-operations)).
</Callout>

### Plugin options

```ts
import type { DbPluginOptions } from "@dimah-s3/db";
```
...
db({
  client: dimahS3Db,
  resolveScope,
  deleteMode: "soft", // [!code highlight]
});
```
...
<Accordion title="When should I use hard delete?">

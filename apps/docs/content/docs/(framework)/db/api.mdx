...
Register `dbClient()` once. In a client component, `s3Client.useApi()` gives you `api.db.listObjects`:
...
Quotas are app-owned. The db plugin enforces ownership first; your guards run **after** ([hook order](/docs/server/plugins#hook-order)).

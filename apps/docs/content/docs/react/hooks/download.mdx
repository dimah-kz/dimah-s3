...
export function AvatarImage({ avatarKey }: { avatarKey: string }) {
  ...
  if (isLoading)
    return <div className="bg-muted size-16 animate-pulse rounded-full" />;
  if (!url) return null;
...
<Accordion title="How do I set a custom download filename?">

Pass the filename as the second argument to `download` or `presign`:

```tsx
await download(avatarKey, "custom-avatar-name.png"); // [!code highlight]
```

</Accordion>

<Accordion title="Can multiple buttons share one useDownload instance?">
...
It caches URLs in memory by route, key, disposition, and filename. Presigned
URLs are reused until 15 seconds before their `expiresIn` window; proxy URLs
with `expiresIn: 0` are cached for up to 24 hours. Calling `refresh()`
invalidates the entry and requests a fresh URL.

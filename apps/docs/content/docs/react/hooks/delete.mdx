...
`useDelete` coordinates object deletion with your server API. It supports
two-step confirmation, immediate deletion, and batch operations.
...
await remove(avatarKey); // [!code highlight]
```

### 3. Batch deletion (`removeMany`)

Delete multiple keys in one dimah-s3 API request. The server authorizes and
deletes each object independently, so the result can contain partial failures:
...
```ts
import type { UseDeleteOptions, UseDeleteReturn } from "@dimah-s3/react";
```
...
Enforce ownership on the server in `delete.guard`; client-side checks are not
an authorization boundary. The default `keyPrefix` only keeps keys inside this
route's folder — it does not know which user owns a file. See

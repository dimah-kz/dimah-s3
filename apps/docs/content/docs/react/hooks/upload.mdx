...
export function FileUploader() {
  const upload = useUpload({
    route: "avatar",
    onSuccess: (results) => {
      console.log("Uploaded object key:", results[0]?.key); // [!code highlight]
    },
  });
  ...
  return (
    <div
      {...upload.getRootProps()}
      className="hover:border-primary cursor-pointer rounded-lg border border-dashed p-6 text-center transition-colors"
    >
...
const upload = useUpload({
  route: "avatar",
  noClick: true, // Prevents container click from opening file picker // [!code ++]
});
...
```ts
import type { UseUploadOptions, UseUploadReturn } from "@dimah-s3/react";
```
...
<Accordion title="What happens if the route catalog fails to load?">

`useUpload` still works because the server enforces type and size on presign.
...
Read `previewUrl` from an item in `upload.files`; `upload.file?.previewUrl` is
the single-file shorthand:

```tsx
export function AvatarWithPreview() {
  const upload = useUpload({ route: "avatar" });
  const preview = upload.file?.previewUrl; // [!code highlight]
...
const upload = useUpload({
  ...
  getUploadOptions: (file) => ({
    ...
    metadata: {
      ...
      uploadedAt: new Date().toISOString(), // [!code ++]
    }, // [!code ++]
  }), // [!code ++]
});
```
...
Call `cancel()` to abort the upload and clean up temporary parts. To stop a
stored multipart upload without aborting it, call `detach()` and resume it on a
later visit:

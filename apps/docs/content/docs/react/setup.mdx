...
Using `@dimah-s3/ui`? Also mount `<Toaster />` from `@dimah-s3/ui` (or `@/components/ui/toast` for registry) next to the provider — see [Setup](/docs/react/ui/setup).
...
Hook reference: [Upload](/docs/react/hooks/upload), [Download](/docs/react/hooks/download), [Delete](/docs/react/hooks/delete). UI components: [Setup](/docs/react/ui/setup).

---
title: Custom UI
description: Build bespoke upload interfaces using headless hooks and custom components.
---

import { CustomUploadDemo } from "@/components/demos/custom-upload-demo";

Combine `@dimah-s3/react` hooks with your own components or custom shadcn primitives.

## Preview

<DemoPreview name="custom-upload-demo.tsx">
  <CustomUploadDemo />
</DemoPreview>

## Example: Headless File Uploader

```tsx
"use client";

import { useUpload } from "@dimah-s3/react";
import { Button } from "@/components/ui/button";

export function CustomFilePicker() {
  const upload = useUpload({
    route: "avatar",
    noDrag: true, // [!code highlight]
    noClick: true, // [!code highlight]
  });

  return (
    <div className="flex items-center gap-4">
      <input {...upload.getInputProps()} />
      <Button type="button" onClick={() => upload.open()}>
        {upload.isUploading
          ? `Uploading ${upload.progress.percent}%`
          : "Choose file"}
      </Button>
    </div>
  );
}
```

## Frequently asked questions

<Accordions>

<Accordion title="How do I access low-level transfer metrics?">

Read `upload.progress` for loaded bytes, total bytes, percentage, and transfer
speed:

```tsx
const { progress } = useUpload({ route: "avatar" });

console.log(progress.loaded, progress.total, progress.percent);
```

</Accordion>

</Accordions>

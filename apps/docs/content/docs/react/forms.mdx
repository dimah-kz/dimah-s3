...
Write each key into the form from `onFileSuccess`. At that point the server has
accepted that object and verified it with `HeadObject`. The batch-level
`onSuccess` callback runs only when every selected file succeeds.
...
<Flow
  ...
  steps={[
    ...
    { name: "onFileSuccess", kind: "hook", note: "one confirmed UploadResult" },
    {
...
If a batch partially fails, successful objects remain confirmed and their keys
stay in the form. Let the user retry the failed files, or explicitly delete the
successful objects when discarding the whole batch.
...
Append each confirmed result in `onFileSuccess` so partial batch success is not
lost:

```ts
onFileSuccess: (_file, result) => {
  const keys = form.getValues("objectKeys");
  form.setValue("objectKeys", [...keys, result.key]); // [!code highlight]
};
```
...
export function AttachmentForm() {
  ...
  const upload = useUpload({
    ...
    onFileSuccess: (_file, result) => {
      const keys = form.getValues("objectKeys");
      form.setValue("objectKeys", [...keys, result.key], {
        // [!code highlight]
        shouldDirty: true,
        shouldValidate: true,
      });
...
export function AttachmentForm() {
  ...
  const upload = useUpload({
    ...
    maxFiles: 5,
    onFileSuccess: (_file, result) => {
      form.setFieldValue("objectKeys", (keys) => [...keys, result.key]); // [!code highlight]
    },
  });
...
const upload = useUpload({
  route: "avatar",
  maxFiles: 1, // [!code highlight]
  onSuccess: (results) => {
    form.setValue("avatarKey", results[0]?.key ?? null); // [!code highlight]
  },
});
```
...
2. Set values from confirmed results: `onFileSuccess` for batches or
   `onSuccess` for an all-or-nothing single-file flow.
...
[`purgeStalePendingObjects`](/docs/db/purge) only cleans up uploads that never
completed S3 confirmation. It does not remove successfully uploaded but

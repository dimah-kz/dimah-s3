...
<Callout title="Uploading more than 5 GiB">
  A single presigned POST cannot exceed **5 GiB**. For larger files, enable
  `upload.multipart` or set `upload.method` to `"PUT"` when your storage
  provider supports a single PUT of that size. The route's `maxFileSize` still
  applies.
</Callout>
...
createServer((req, res) => {
  if (req.url?.startsWith("/api/s3")) return handler(req, res); // [!code highlight]
  res.statusCode = 404;
  res.end("Not found");
}).listen(3000);
```
...
export async function getAvatarDownloadUrl(key: string) {
  return s3.api.download({
    ...
    headers: await headers(), // [!code highlight]
  });
}
```
...
```ts
import type { DimahS3Config } from "@dimah-s3/server";
```
...
By default, dimah-s3 isolates object keys by route prefix (the route name):

```
avatar/{uuid}/{fileName}
```

Upload generates that key. Confirm, download, and delete accept a client-sent
key only if it stays under the same prefix; otherwise they return
`INVALID_KEY`. That check is always in `@dimah-s3/server` — [`db()`](/docs/db)
adds per-user scope, not the prefix bound.

Setting `keyPrefix: false` disables that namespace boundary and requires strict
application guards. See
...
export const s3Client = createS3Client({
  baseURL: "https://api.example.com/api/storage", // [!code ++]
});
```

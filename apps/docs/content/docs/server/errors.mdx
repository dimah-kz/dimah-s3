...
Use the `errors` helper to throw standard library errors. A plain `Error` has
no stable `code` — in guards it becomes a generic 403:

```ts
import { APIError, errors, isAPIError, isS3ErrorCode } from "@dimah-s3/server";

throw new Error("Not allowed"); // [!code --]
throw errors.unauthorized(); // [!code ++]
...
throw errors.fileTypeNotAllowed("archive.zip", "application/zip");
...
| `QUOTA_EXCEEDED`         | 413  | `db()` or `createQuotaGuard` rejects a scope's byte or file-count projection   |

...
Set `upload.maxFileSize`; the server checks each signed part against the
remaining allowance and sums uploaded part sizes before
`CompleteMultipartUpload`. If the total size exceeds the limit, the server
aborts the multipart upload and returns `PAYLOAD_TOO_LARGE`.

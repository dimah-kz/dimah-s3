...
Use `definePlugin` to create reusable server extensions such as
[`db()`](/docs/db). Plugins can contribute lifecycle hooks, HTTP endpoints
under your `basePath`, and typed instance context (`s3[id]`).
...
export const s3 = dimahS3({
  ...
  plugins: [auditPlugin], // [!code ++]
  routes: {
    avatar: route({ upload: true }),
  },
});

s3.audit.log("Storage ready"); // [!code ++]
```
...
<Accordion title="How do I add client-side actions for a server plugin?">

Create a client plugin with `defineClientPlugin` from `@dimah-s3/core`:

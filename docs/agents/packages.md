...
4. `guard` / `*Guard` / `on*` hooks for consumer auth and side effects only. Route `prefix` (string or `ObjectContext` factory) / `object` generate keys and S3 metadata on upload. The client does not send `key` on upload, or `bucket` / `acl` / `expiresIn`. Add named policies with `route()` (`packages/server/src/route.ts`).

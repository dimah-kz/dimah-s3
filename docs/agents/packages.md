...
4. Required `route` (`S3RouteName`) on hooks that hit core endpoints. Public upload is `useUpload` only (`maxFiles` for a batch; UI takes the return as `upload`). Download / delete UI take `useDownload` / `useDelete` as `download` / `delete`; `objectKey` stays on the control. The hook return also exposes `objectKey` for the active operation so a list can share one instance. Upload does not take `objectKey`. Omit `accept` / `maxFileSize` / `multipart` / `checksum` to fill them from `api.catalog()`. Augment `DimahS3Routes` so route names are not a free `string`.

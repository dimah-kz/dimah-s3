...
3. Feature flags via named `routes` (`upload`, `download`, `delete` on each route; multipart under `upload.multipart`) — disabled → `FEATURE_DISABLED` (HTTP 404; endpoint still registered). `true` or an options object means on; omit or `false` means off. Upload defaults on; `download` / `delete` / `upload.multipart` default off.
4. `guard` / `*Guard` / `on*` hooks for consumer auth and side effects only. `upload.object` generates keys and S3 metadata on upload (`{ prefix?, key?, metadata?, acl? }`). Keys are nested under route `keyPrefix` (default: the route name). Default key is `{keyPrefix}/{uuid}/{name}`. Confirm / download / delete / multipart follow-ups reject keys outside that prefix (`INVALID_KEY`). `keyPrefix: false` disables the bound. The client does not send `key` on upload, or `bucket` / `acl` / `expiresIn`. Multipart `signPart` requires `partSize` (locked into the URL). When `maxFileSize` is set, signPart also rejects a part that would push listed uploaded bytes over the cap; complete aborts the MPU if listed parts exceed it. Add named policies with `route()` (`packages/server/src/route.ts`).
...
1. Plugin subsystem lives in `packages/server/src/plugin/` (`DimahS3Plugin`, `definePlugin`, `applyPlugins`, `chainHooks`, `FEATURE_HOOK_KEYS`, `MULTIPART_HOOK_KEYS`). Endpoints use `createS3Endpoint` from `@dimah-s3/server` / `@dimah-s3/server/api`.

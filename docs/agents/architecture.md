...
4. Config is route-only: `dimahS3({ routes })` is required. The instance holds `client` / `bucket` / `plugins`; each `route()` nests `upload` / `download` / `delete`. Upload owns `fileTypes` and `object`. The client sends a route name; the server owns keys (`object` or `{route}/{uuid}/{name}`).
5. Multipart is opt-in per route (`multipart: true`). The client threshold (~50 MB) is internal — not a consumer knob.

...
4. Config is route-only: `dimahS3({ routes })` is required. The instance holds `client` / `bucket` / `plugins`; each `route()` is a nested mini-config. The client sends a route name; the server owns keys (`prefix` / `resolveKey`).

...
4. Config is route-only: `dimahS3({ routes })` is required. The instance holds `client` / `bucket` / `plugins`; each `route()` nests `upload` / `download` / `delete`. Upload owns `fileTypes`, `prefix`, and `object`. The client sends a route name; the server owns keys (`prefix` / `object`).
